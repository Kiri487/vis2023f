...
function updateIconPosition(floor, x, y) {
    ...
    const containerHeight = floorContainer.offsetHeight;
    const pixelX = (iconX / 100) * containerWidth;
    const pixelY = (iconY / 100) * containerHeight;

    // Draw movement traces
    const tracePoint = document.createElement("div");
    tracePoint.className = "trace-point";
    tracePoint.style.left = `${pixelX - 1}px`;
    tracePoint.style.top = `${pixelY + 14}px`;
    floorContainer.appendChild(tracePoint);
